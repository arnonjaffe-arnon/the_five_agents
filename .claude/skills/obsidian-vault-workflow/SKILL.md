...
description: Manage a structured Obsidian vault as Claude Code's long-term memory. Use for every task that reads or writes files or makes decisions — reads vault context BEFORE starting (Phase 1) and appends a session log entry AFTER completing (Phase 2). Triggers: vault, obsidian memory, session log, meeting notes, content brief, brand guidelines, topic file.
---

# Obsidian Vault Workflow Skill

The project vault at `vault/` is Claude Code's long-term memory. It is organized **one file per topic** — the file contains a persistent Overview (what the topic is) plus a Session Log of dated compact summaries. Reading the relevant topic file before work and appending a session entry after work is **mandatory**.
...
Every `###` session heading ends with a bracketed status tag:
...
If the status changes later, do NOT rewrite the old heading — add a new dated entry with the new status.

### Open Questions section

Rules:
- Add an entry whenever this session ended with an unresolved decision, untested assumption, deferred follow-up, or known limitation.
- **Remove** entries when they get resolved (don't just leave them crossed out).
- If nothing is open, write `- none`.
- Keep entries short (one line each). Link with `[[wikilink#YYYY-MM-DD]]` if context is needed.

### 4. Overview update rule

Update the Overview **only** when this session:
- Expanded or narrowed the topic's scope
- Added a major component, module, or flow
- Corrected prior understanding
- Changed status (e.g. "planned" → "shipped")

If none apply, leave the Overview untouched. When updating, integrate with existing text rather than replacing it wholesale.

### 5. Append the dated Session Log entry

Add a new `### YYYY-MM-DD — <short title>` section at the bottom of `## Session Log`. Date comes from the session's `currentDate` context.

### 6. Wikilinks — non-negotiable

The `- **Related:**` line MUST contain `[[wikilinks]]` to:
- Any topic file you read in Phase 1
- Prior notes on the same module, channel, or feature area
- Briefs or guidelines referenced while working
...
- New `### YYYY-MM-DD` section appended at the **bottom** of `## Session Log`
...
Inside each folder, one `.md` per topic, plus an `_index.md` listing every topic with a one-line description.
...
When you create a new topic file, immediately add a line to the matching folder's `_index.md`. When you delete or rename a topic, update the index.

---

## Anti-Patterns (Do Not Do These)

- ❌ Creating `YYYY-MM-DD <topic>.md` style dated filenames
- ❌ Omitting the `[status]` tag on a session heading
- ❌ Leaving resolved items in `## Open Questions`
- ❌ Creating a topic file without adding its line to the folder's `_index.md`
- ❌ Creating a new topic file when a close-match file exists, without first asking the user
...
- ❌ Dumping a diff or full commit message into the entry
- ❌ Creating the note outside `vault/`
...
- [ ] Opened the folder's `_index.md` to find the topic
- [ ] Looked for exact filename match; handled close-match case
- [ ] If topic file exists → Read it fully
...
- [ ] If new file → wrote Overview + Open Questions + first Session Log entry; added line to `_index.md`
